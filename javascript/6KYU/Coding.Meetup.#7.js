...
const findSenior2 = (list) => {
    return list.filter((devs) => devs.age === list.reduce((acc, devs) => devs.age > acc ? devs.age : acc, 0));
}

//Another solution
const findSenior3 = (list) => {
    let maxAge = Math.max(...list.map((devs) => devs.age));
    return list.filter((devs) => devs.age === maxAge);
}
